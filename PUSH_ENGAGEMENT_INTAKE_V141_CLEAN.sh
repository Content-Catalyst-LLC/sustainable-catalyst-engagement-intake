...
set -Eeuo pipefail
IFS=$'\n\t'
VERSION="1.4.1"; SLUG="sustainable-catalyst-engagement-intake"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v141-push.XXXXXX")"; BACKUPS="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"; STAMP="$(date +%Y%m%d-%H%M%S)"
cleanup(){ rm -rf "$WORK"; }; trap cleanup EXIT; trap 'rc=$?; echo "ERROR: v1.4.1 update failed at line $LINENO (exit $rc)." >&2; exit $rc' ERR
...
[[ -f "$MAIN" && -f "$README" && -f "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" && -f "$SRC/tests/proposal-reliability.php" ]] || fail 'Invalid v1.4.1 repository archive.'
grep -Fq 'Version:     1.4.1' "$MAIN" || fail 'Plugin version marker missing.'
grep -Fq "SC_EI_DB_VERSION', '1.4.0'" "$MAIN" || fail 'Database version marker missing.'
grep -Fq "SC_EI_PLATFORM_SCHEMA_VERSION', '1.4.1'" "$MAIN" || fail 'Platform schema marker missing.'
grep -Fq "SC_EI_PROPOSAL_SCHEMA_VERSION', '1.0.1'" "$MAIN" || fail 'Proposal schema marker missing.'
grep -Fq 'Stable tag: 1.4.1' "$README" || fail 'Stable tag missing.'
grep -Fq "PATCH_MIGRATION_KEY = 'v1_4_1_proposal_versioning_approval_reliability'" "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" || fail 'v1.4.1 patch migration missing.'
grep -Fq 'verify_approval_integrity' "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" || fail 'Approval integrity verifier missing.'
grep -Fq 'rollback_proposal_sender_response' "$SRC/$SLUG/includes/class-sc-ei-workflow-repository.php" || fail 'Proposal response compensation missing.'
grep -Fq 'consistency_metrics' "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" || fail 'Proposal consistency metrics missing.'
grep -Fq 'ensure_conversion_receipt' "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" || fail 'Engagement conversion repair missing.'
...
git add -A
if git diff --cached --quiet; then echo 'No changes detected.'; else git commit -m 'Build v1.4.1 Proposal Versioning, Approval, and Engagement Conversion Reliability Patch'; fi
