...
set -Eeuo pipefail
IFS=$'\n\t'
VERSION="1.7.0"; SLUG="sustainable-catalyst-engagement-intake"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v170-push.XXXXXX")"; BACKUPS="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"; STAMP="$(date +%Y%m%d-%H%M%S)"
cleanup(){ rm -rf "$WORK"; }; trap cleanup EXIT; trap 'rc=$?; echo "ERROR: v1.7.0 update failed at line $LINENO (exit $rc)." >&2; exit $rc' ERR
...
[[ -f "$MAIN" && -f "$README" && -f "$SRC/$SLUG/includes/class-sc-ei-billing-repository.php" && -f "$SRC/tests/billing-invoicing-payment-handoffs.php" ]] || fail 'Invalid v1.7.0 repository archive.'
grep -Fq 'Version:     1.7.0' "$MAIN" || fail 'Plugin version marker missing.'
grep -Fq "SC_EI_DB_VERSION', '1.7.0'" "$MAIN" || fail 'Database version marker missing.'
grep -Fq "SC_EI_PLATFORM_SCHEMA_VERSION', '1.7.0'" "$MAIN" || fail 'Platform schema marker missing.'
grep -Fq "SC_EI_PORTAL_SCHEMA_VERSION', '1.8.0'" "$MAIN" || fail 'Portal schema marker missing.'
grep -Fq "SC_EI_BILLING_SCHEMA_VERSION', '1.0.0'" "$MAIN" || fail 'Billing schema marker missing.'
grep -Fq "SC_EI_SERVICE_INTELLIGENCE_SCHEMA_VERSION', '1.0.0'" "$MAIN" || fail 'Service Intelligence schema marker missing.'
grep -Fq 'Stable tag: 1.7.0' "$README" || fail 'Stable tag missing.'
grep -Fq "MIGRATION_KEY = 'v1_7_0_billing_invoicing_payment_handoffs'" "$SRC/$SLUG/includes/class-sc-ei-billing-repository.php" || fail 'v1.7.0 billing migration missing.'
grep -Fq "PAYMENT_HANDOFF_SCHEMA = 'sc-payment-handoff/1.0'" "$SRC/$SLUG/includes/class-sc-ei-billing-schema.php" || fail 'Payment-handoff schema missing.'
grep -Fq 'payment instruments forbidden' "$SRC/tests/billing-invoicing-payment-handoffs.php" || fail 'Billing privacy regression coverage missing.'
grep -Fq 'billing_invoicing_payment_handoffs' "$SRC/$SLUG/includes/class-sc-ei-platform-validation.php" || fail 'Live Validation billing coverage missing.'
grep -Fq 'sender_projection' "$SRC/$SLUG/includes/class-sc-ei-billing-repository.php" || fail 'Sender-safe billing projection missing.'
...
git add -A
if git diff --cached --quiet; then echo 'No changes detected.'; else git commit -m 'Build v1.7.0 Billing, Invoicing, and Payment Handoffs'; fi
