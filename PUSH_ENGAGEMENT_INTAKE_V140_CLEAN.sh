...
set -Eeuo pipefail
IFS=$'\n\t'
VERSION="1.4.0"; SLUG="sustainable-catalyst-engagement-intake"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v140-push.XXXXXX")"; BACKUPS="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"; STAMP="$(date +%Y%m%d-%H%M%S)"
cleanup(){ rm -rf "$WORK"; }; trap cleanup EXIT; trap 'rc=$?; echo "ERROR: v1.4.0 update failed at line $LINENO (exit $rc)." >&2; exit $rc' ERR
...
[[ -f "$MAIN" && -f "$README" && -f "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" && -f "$SRC/tests/proposal-governance.php" ]] || fail 'Invalid v1.4.0 repository archive.'
grep -Fq 'Version:     1.4.0' "$MAIN" || fail 'Plugin version marker missing.'
grep -Fq "SC_EI_DB_VERSION', '1.4.0'" "$MAIN" || fail 'Database version marker missing.'
grep -Fq "SC_EI_PLATFORM_SCHEMA_VERSION', '1.4.0'" "$MAIN" || fail 'Platform schema marker missing.'
grep -Fq "SC_EI_PROPOSAL_SCHEMA_VERSION', '1.0.0'" "$MAIN" || fail 'Proposal schema marker missing.'
grep -Fq 'Stable tag: 1.4.0' "$README" || fail 'Stable tag missing.'
grep -Fq "MIGRATION_KEY = 'v1_4_0_proposals_statements_of_work_engagement_approvals'" "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" || fail 'v1.4.0 migration missing.'
grep -Fq 'proposal_approvals' "$SRC/$SLUG/includes/class-sc-ei-database.php" || fail 'Approval table missing.'
grep -Fq 'statement_of_work_versions' "$SRC/$SLUG/includes/class-sc-ei-database.php" || fail 'SOW version table missing.'
grep -Fq 'change_requests' "$SRC/$SLUG/includes/class-sc-ei-database.php" || fail 'Change request table missing.'
grep -Fq 'convert_to_engagement' "$SRC/$SLUG/includes/class-sc-ei-proposal-governance-repository.php" || fail 'Engagement conversion missing.'
...
git add -A
if git diff --cached --quiet; then echo 'No changes detected.'; else git commit -m 'Build v1.4.0 Proposals, Statements of Work, and Engagement Approvals'; fi
