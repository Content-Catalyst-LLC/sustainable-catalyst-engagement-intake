...
set -Eeuo pipefail
IFS=$'\n\t'
VERSION="2.0.0"; SLUG="sustainable-catalyst-engagement-intake"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v200-push.XXXXXX")"; BACKUPS="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"; STAMP="$(date +%Y%m%d-%H%M%S)"
cleanup(){ rm -rf "$WORK"; }; trap cleanup EXIT; trap 'rc=$?; echo "ERROR: v2.0.0 update failed at line $LINENO (exit $rc)." >&2; exit $rc' ERR
...
[[ -f "$MAIN" && -f "$README" && -f "$SRC/$SLUG/includes/class-sc-ei-unified-platform-repository.php" && -f "$SRC/tests/integrated-platform-v2.php" ]] || fail 'Invalid v2.0.0 repository archive.'
grep -Fq 'Version:     2.0.0' "$MAIN" || fail 'Plugin version marker missing.'
...
grep -Fq 'Stable tag: 2.0.0' "$README" || fail 'Stable tag missing.'
grep -Fq "MIGRATION_KEY = 'v2_0_0_integrated_advisory_support_institutional_platform'" "$SRC/$SLUG/includes/class-sc-ei-unified-platform-repository.php" || fail 'v2.0.0 integrated-platform migration missing.'
...
git add -A
if git diff --cached --quiet; then echo 'No changes detected.'; else git commit -m 'Build v2.0.0 Integrated Advisory, Support, and Institutional Engagement Platform'; fi
