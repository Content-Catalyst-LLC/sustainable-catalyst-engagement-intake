...
set -Eeuo pipefail
IFS=$'\n\t'
VERSION="2.0.2"; SLUG="sustainable-catalyst-engagement-intake"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v202-push.XXXXXX")"; BACKUPS="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"; STAMP="$(date +%Y%m%d-%H%M%S)"
cleanup(){ rm -rf "$WORK"; }; trap cleanup EXIT; trap 'rc=$?; echo "ERROR: v2.0.2 update failed at line $LINENO (exit $rc)." >&2; exit $rc' ERR
...
[[ -f "$MAIN" && -f "$README" && -f "$SRC/$SLUG/includes/class-sc-ei-unified-platform-repository.php" && -f "$SRC/tests/integrated-platform-v2.php" ]] || fail 'Invalid v2.0.2 repository archive.'
grep -Fq 'Version:     2.0.2' "$MAIN" || fail 'Plugin version marker missing.'
...
grep -Fq 'Stable tag: 2.0.2' "$README" || fail 'Stable tag missing.'
...
grep -Fq 'create_table_if_missing( $service_intelligence_findings, $sql_service_intelligence_findings )' "$SRC/$SLUG/includes/class-sc-ei-database.php" || fail 'Service intelligence recovery missing.'
grep -Fq 'create_table_if_missing( $payment_handoffs, $sql_payment_handoffs )' "$SRC/$SLUG/includes/class-sc-ei-database.php" || fail 'Payment handoff recovery missing.'
...
git add -A
if git diff --cached --quiet; then echo 'No changes detected.'; else git commit -m 'Build v2.0.2 Reserved-Identifier Table Recovery'; fi
