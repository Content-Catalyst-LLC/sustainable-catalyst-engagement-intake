...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v103-push.XXXXXX")"
BACKUP_ROOT="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"

cleanup() { rm -rf "$WORK"; }
trap cleanup EXIT
trap 'rc=$?; echo; echo "ERROR: v1.0.3 repository update failed on line $LINENO (exit $rc)." >&2; exit $rc' ERR
say() { printf '\n==> %s\n' "$1"; }
...
REPOSITORY="$SRC/$SLUG/includes/class-sc-ei-platform-repository.php"
VALIDATION="$SRC/$SLUG/includes/class-sc-ei-platform-validation.php"
PILOT="$SRC/$SLUG/includes/class-sc-ei-pilot-operations.php"
PUBLIC_JS="$SRC/$SLUG/assets/js/public.js"
[[ -f "$MAIN" && -f "$README" && -f "$REPOSITORY" && -f "$VALIDATION" && -f "$PILOT" && -f "$PUBLIC_JS" ]] || fail "Required v1.0.3 files are missing."
...
grep -Fq "LAUNCH_MIGRATION_KEY = 'v1_0_3_pilot_findings_public_launch_hardening'" "$REPOSITORY" || fail "v1.0.3 launch migration is missing."
grep -Fq "SC_EI_Pilot_Operations::pilot_complete_and_fresh()" "$REPOSITORY" || fail "Pilot evidence production gate is missing."
grep -Fq "SC_EI_Pilot_Operations::external_mail_confirmed_and_fresh()" "$REPOSITORY" || fail "External mail evidence production gate is missing."
grep -Fq "runtime_security_probe" "$VALIDATION" || fail "Runtime upload-security validation is missing."
grep -Fq "sessionStorage" "$PUBLIC_JS" || fail "Browser-tab draft recovery is missing."
grep -Fq "100 === \$score && empty( \$required_failures ) && empty( \$warnings )" "$REPOSITORY" || fail "Strict production gate is missing."
...
if git diff --cached --quiet; then
  ...
  git commit -m "Build v1.0.3 Pilot Findings and Public Launch Hardening"
fi
