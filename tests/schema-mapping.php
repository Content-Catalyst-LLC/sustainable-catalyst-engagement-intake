<?php
/**
 * Static v0.9.0 schema, repository, and privacy mapping checks.
 */
...
foreach ( array(
	...
	$missing = array();
	foreach ( $columns as $column ) {
		if ( false === strpos( $workflow_repository, "'" . $column . "'" ) ) {
			$missing[] = $column;
		}
	}
	if ( $missing ) {
...
foreach ( array( $inquiries, $attachments, $reviews, $fit_repository, $portal_repository, $workflow_repository, $communications, $templates, $privacy_repository, $policy_repository ) as $source ) {
...
echo "Engagement Intake v0.9.0 schema checks passed.\n";
