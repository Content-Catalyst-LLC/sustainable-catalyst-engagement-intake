...
$checks = array(
 'v0.11.0 markers' => strpos($main,'Version:     0.11.0')!==false && strpos($main,"SC_EI_DB_VERSION', '0.11.0'")!==false && strpos($main,"SC_EI_ANALYTICS_SCHEMA_VERSION', '1.0.0'")!==false,
...
$failed=array_keys(array_filter($checks,fn($v)=>!$v)); if($failed){fwrite(STDERR,'Analytics schema checks failed: '.implode(', ',$failed).PHP_EOL);exit(1);} foreach($checks as $k=>$v) echo 'PASS: '.$k.PHP_EOL; echo "Engagement Intake v0.11.0 analytics schema fixtures passed.\n";
