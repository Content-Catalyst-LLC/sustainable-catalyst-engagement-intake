...
$checks=array(
 ...
 'readme stable tag'=>strpos($readme,'Stable tag: 1.0.0')!==false,
...
$failed=array_keys(array_filter($checks,fn($v)=>!$v));if($failed){fwrite(STDERR,'Unified release checks failed: '.implode(', ',$failed).PHP_EOL);exit(1);}foreach($checks as $k=>$v)echo 'PASS: '.$k.PHP_EOL;echo "Unified Contact and Engagement Platform v1.0.0 release checks passed.\n";
