...
$checks=array(
 'v0.12.0 markers'=>strpos($main,'Version:     0.12.0')!==false && strpos($main,"SC_EI_DB_VERSION', '0.12.0'")!==false && strpos($main,"SC_EI_HARDENING_SCHEMA_VERSION', '1.0.0'")!==false,
...
$failed=array_keys(array_filter($checks,fn($v)=>!$v)); if($failed){fwrite(STDERR,'Hardening schema checks failed: '.implode(', ',$failed).PHP_EOL);exit(1);} foreach($checks as $k=>$v) echo 'PASS: '.$k.PHP_EOL; echo "Engagement Intake v0.12.0 hardening schema fixtures passed.\n";
