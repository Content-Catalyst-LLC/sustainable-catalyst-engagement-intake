...
DIST="$ROOT/dist"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v150-package.XXXXXX")"
...
bash -n "$0"
bash -n PUSH_ENGAGEMENT_INTAKE_V150_CLEAN.sh
...
for block in text.split('=== '):
    if 'PASS:' not in block:
        m=re.findall(r'\((\d+) assertions\)',block)
        if m: passes += int(m[-1])
manifest={
 'name':'Sustainable Catalyst Contact and Engagement Platform','version':version,
 'release':'Secure Client Workspace and Collaboration','plugin_slug':'sustainable-catalyst-engagement-intake',
 'requires_wordpress':'6.5','requires_php':'8.1','database_version':'1.5.0',
 'schemas':{'review':'1.0.0','communication':'1.0.0','privacy':'1.0.0','fit':'1.0.0','portal':'1.7.0','workflow':'1.3.0','graph':'1.0.0','engagement':'1.2.0','analytics':'1.0.0','hardening':'1.0.0','workflow_core':'1.0.0','platform':'1.5.0','lifecycle':'1.0.0','support':'1.0.1','calendar':'1.0.1','proposal_governance':'1.0.1','workspace':'1.0.0'},
 'migration_keys':['v1_0_0_unified_contact_engagement_platform','v1_0_2_production_readiness_live_validation','v1_0_3_pilot_findings_public_launch_hardening','v1_1_0_advisory_operations_engagement_lifecycle','v1_1_1_inquiry_persistence_lifecycle_reliability','v1_2_0_support_operations_product_intelligence','v1_2_1_support_operations_cross_product_reliability','v1_3_0_microsoft_teams_calendar_coordination','v1_3_1_scheduling_reminder_timezone_reliability','v1_4_0_proposals_statements_of_work_engagement_approvals','v1_4_1_proposal_versioning_approval_reliability','v1_5_0_secure_client_workspace_collaboration'],
 ...
 'secure_client_workspace':{
   'schema':'sc-client-workspace/1.0','tables':['client_workspaces','workspace_members','workspace_milestones','workspace_deliverables','workspace_messages','workspace_documents','workspace_events'],
   'engagement_linked':True,'explicit_membership':True,'raw_sender_email_stored':False,'sender_projection_allowlist':True,'protected_document_relationships':True,'same_inquiry_document_enforcement':True,
   'governed_workspace_transitions':True,'sender_deliverable_decisions':True,'workspace_specific_sender_messages':True,'privacy_export_and_redaction':True,'audit_history':True,
   'automatic_contract':False,'electronic_signature':False,'automatic_payment':False,'automatic_scope_expansion':False
 },
 ...
 'live_validation':['database and v1.5.0 workspace migration contract','temporary inquiry and lifecycle transition','support case and cross-product privacy contract','Teams scheduling and reminder workflow','proposal SOW approval conversion and change request workflow','temporary engagement-linked client workspace','explicit sender and staff members','sender-safe workspace activation','milestone and deliverable creation','deliverable publication and sender decision','staff and sender workspace messages','sender projection allowlist','workspace and all related artifact cleanup'],
 'boundaries':{'automatic_launch':False,'automatic_acceptance':False,'automatic_proposal':False,'automatic_contract':False,'electronic_signature':False,'automatic_activation':False,'automatic_payment':False,'automatic_meeting_booking':False,'automatic_workspace_enrollment':False,'automatic_scope_expansion':False},
