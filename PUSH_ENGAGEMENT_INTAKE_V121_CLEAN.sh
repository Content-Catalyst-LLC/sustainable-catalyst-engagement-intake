...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-ei-v121-push.XXXXXX")"
BACKUP_ROOT="${SC_EI_BACKUP_DIR:-$HOME/Downloads/${SLUG}-local-backups}"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"

cleanup() { rm -rf "$WORK"; }
trap cleanup EXIT
trap 'rc=$?; echo; echo "ERROR: v1.2.1 repository update failed on line $LINENO (exit $rc)." >&2; exit $rc' ERR
say() { printf '\n==> %s\n' "$1"; }
...
[[ -f "$MAIN" && -f "$README" && -f "$LIFECYCLE" && -f "$LIFECYCLE_SCHEMA" && -f "$LIFECYCLE_ADMIN" && -f "$DATABASE" && -f "$PLATFORM" && -f "$PORTAL" && -f "$SUPPORT" && -f "$SUPPORT_SCHEMA" && -f "$SUPPORT_ADMIN" ]] || fail "Required v1.2.1 files are missing."
...
grep -Fq "SC_EI_PLATFORM_SCHEMA_VERSION', '1.2.1'" "$MAIN" || fail "Platform evidence schema marker is not 1.2.1."
...
if git diff --cached --quiet; then
  ...
  git commit -m "Build v1.2.1 Support Operations and Cross-Product Reliability Patch"
fi
