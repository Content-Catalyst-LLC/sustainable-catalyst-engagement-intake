...
final class SC_EI_Admin {
	...
	public static function menu(): void {
		...
		SC_EI_Command_Center_Admin::submenu();
	...
	public static function default_settings(): array {
		return array_merge(
			...
			SC_EI_Unified_Platform_Schema::default_settings()
		);
	}
	...
	public static function sanitize_settings( $value ): array {
		...
		return array(
			...
			'analytics_intelligence_enabled'          => array_key_exists( 'analytics_intelligence_enabled', $value ) ? ( empty( $value['analytics_intelligence_enabled'] ) ? 0 : 1 ) : absint( $current['analytics_intelligence_enabled'] ),
			'analytics_auto_candidate_findings'       => 0,
			'analytics_finding_review_days'           => max( 1, min( 180, absint( $value['analytics_finding_review_days'] ?? $current['analytics_finding_review_days'] ) ) ),
			'analytics_snapshot_fresh_days'            => max( 1, min( 90, absint( $value['analytics_snapshot_fresh_days'] ?? $current['analytics_snapshot_fresh_days'] ) ) ),
			'analytics_intelligence_retention_days'    => max( 90, min( 3650, absint( $value['analytics_intelligence_retention_days'] ?? $current['analytics_intelligence_retention_days'] ) ) ),
			'analytics_include_support_intelligence'   => array_key_exists( 'analytics_include_support_intelligence', $value ) ? ( empty( $value['analytics_include_support_intelligence'] ) ? 0 : 1 ) : absint( $current['analytics_include_support_intelligence'] ),
			'analytics_include_workspace_intelligence' => array_key_exists( 'analytics_include_workspace_intelligence', $value ) ? ( empty( $value['analytics_include_workspace_intelligence'] ) ? 0 : 1 ) : absint( $current['analytics_include_workspace_intelligence'] ),
			'analytics_include_proposal_intelligence'  => array_key_exists( 'analytics_include_proposal_intelligence', $value ) ? ( empty( $value['analytics_include_proposal_intelligence'] ) ? 0 : 1 ) : absint( $current['analytics_include_proposal_intelligence'] ),
