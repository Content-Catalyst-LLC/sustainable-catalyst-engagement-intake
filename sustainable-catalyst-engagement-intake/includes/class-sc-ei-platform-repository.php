...
final class SC_EI_Platform_Repository {
	...
	public static function maybe_upgrade(): void {
		...
		SC_EI_Unified_Platform_Repository::record_migration( (string) get_option( 'sc_ei_unified_platform_schema_version_previous', '' ) );
		self::schedule_all();
	}
	...
	public static function readiness(): array {
		...
		$unified_platform_columns = SC_EI_Database::unified_platform_columns_exist();
		...
		$unified_platform_metrics = SC_EI_Unified_Platform_Repository::dashboard();
		$unified_platform_integrity = SC_EI_Unified_Platform_Repository::integrity();
		...
		$checks[] = self::check( 'unified_platform_columns', 'data', __( 'v2.0 canonical dossier and cross-module handoff schema', 'sustainable-catalyst-engagement-intake' ), ! in_array( false, $unified_platform_columns, true ), true, sprintf( '%d/%d', count( array_filter( $unified_platform_columns ) ), count( $unified_platform_columns ) ), 'repair_database' );
		...
		$checks[] = self::check( 'unified_platform_migration_journal', 'data', __( 'v2.0 integrated platform migration journal', 'sustainable-catalyst-engagement-intake' ), self::migration_completed( SC_EI_Unified_Platform_Repository::MIGRATION_KEY ), true, SC_EI_Unified_Platform_Repository::MIGRATION_KEY, 'verify_unified_platform_migration' );
		...
		$intelligence_contract_ok = SC_EI_Service_Intelligence_Schema::SNAPSHOT_SCHEMA === 'sc-engagement-service-intelligence/1.0' && SC_EI_Service_Intelligence_Schema::evidence_is_aggregate( array( 'metric' => 'support_cases', 'count' => 12 ) ) && ! SC_EI_Service_Intelligence_Schema::evidence_is_aggregate( array( 'email' => 'private@example.com' ) );
		...
		$unified_integrity_ok = 0 === array_sum( array_map( 'absint', $unified_platform_integrity ) );
		$checks[] = self::check( 'unified_platform_integrity', 'integrations', __( 'Canonical dossier, relationship, and typed-handoff integrity', 'sustainable-catalyst-engagement-intake' ), $unified_integrity_ok, true, wp_json_encode( $unified_platform_integrity ), 'repair_unified_platform' );
		...
		$timezone_evidence = SC_EI_Calendar_Repository::timezone_runtime_evidence();
		$calendar_integrity_ok = 0 === absint( $calendar_metrics['missing_timezone'] ?? 0 ) && 0 === absint( $calendar_metrics['canceled_active_link'] ?? 0 ) && ! in_array( false, $timezone_evidence, true );
		$checks[] = self::check( 'calendar_integrity', 'integrations', __( 'Calendar time-zone and canceled-link integrity', 'sustainable-catalyst-engagement-intake' ), $calendar_integrity_ok, true, sprintf( '%d missing timezone; %d canceled active link; DST gap %s; repeated hour %s', absint( $calendar_metrics['missing_timezone'] ?? 0 ), absint( $calendar_metrics['canceled_active_link'] ?? 0 ), ! empty( $timezone_evidence['gap_rejected'] ) ? 'rejected' : 'unsafe', ! empty( $timezone_evidence['overlap_rejected'] ) ? 'rejected' : 'unsafe' ), 'review_calendar' );
		$reminder_integrity_ok = 0 === absint( $calendar_metrics['overdue_pending_reminders'] ?? 0 ) && 0 === absint( $calendar_metrics['orphaned_reminders'] ?? 0 ) && 0 === absint( $calendar_metrics['sent_without_communication'] ?? 0 ) && 0 === absint( $calendar_metrics['stale_premeeting_reminders'] ?? 0 ) && 0 === absint( $calendar_metrics['missing_terminal_notice'] ?? 0 );
		$checks[] = self::check( 'calendar_reminder_integrity', 'operations', __( 'Calendar reminder and terminal-notice integrity', 'sustainable-catalyst-engagement-intake' ), $reminder_integrity_ok, true, sprintf( '%d overdue pending; %d orphaned; %d sent without reviewed communication; %d stale pre-meeting; %d missing terminal notice', absint( $calendar_metrics['overdue_pending_reminders'] ?? 0 ), absint( $calendar_metrics['orphaned_reminders'] ?? 0 ), absint( $calendar_metrics['sent_without_communication'] ?? 0 ), absint( $calendar_metrics['stale_premeeting_reminders'] ?? 0 ), absint( $calendar_metrics['missing_terminal_notice'] ?? 0 ) ), 'repair_calendar_reliability' );
		...
		return array(
			'schema'               => 'sc-unified-contact-engagement-platform-readiness/2.0',
			...
			'unified_platform_metrics' => $unified_platform_metrics,
			'unified_platform_integrity' => $unified_platform_integrity,
	...
	public static function platform_summary(): array {
		...
		return array(
			...
			'metrics' => array(
				...
				'analytics'         => SC_EI_Service_Intelligence_Repository::dashboard( 30 ),
	...
	public static function schema_versions(): array {
		return array(
			...
			'billing' => SC_EI_BILLING_SCHEMA_VERSION,
			'unified_platform' => SC_EI_UNIFIED_PLATFORM_SCHEMA_VERSION,
		);
	}
	...
	public static function boundaries(): array {
		return array(
			...
			'automatic_dossier_decisions'     => false,
		);
	}
	...
	public static function repair( string $repair_key, int $actor_user_id ) {
		...
		switch ( $repair_key ) {
			...
			case 'verify_calendar_migration':
				...
				break;
			case 'verify_calendar_reliability_patch':
				$result = SC_EI_Calendar_Repository::record_patch_migration( (string) get_option( 'sc_ei_calendar_schema_version_previous', '' ) );
				break;
			...
			case 'verify_workspace_migration':
				...
				break;
			case 'verify_unified_platform_migration':
				$result = SC_EI_Unified_Platform_Repository::record_migration( (string) get_option( 'sc_ei_unified_platform_schema_version_previous', '' ) );
				break;
			case 'repair_unified_platform':
				SC_EI_Database::install();
				$migration = SC_EI_Unified_Platform_Repository::record_migration( (string) get_option( 'sc_ei_unified_platform_schema_version_previous', '' ) );
				$backfill = SC_EI_Unified_Platform_Repository::backfill( 5000, 0 );
				$result = is_wp_error( $migration ) || ! empty( $backfill['failed'] ) ? new WP_Error( 'unified_platform_repair_incomplete', __( 'The v2.0 dossier repair requires attention.', 'sustainable-catalyst-engagement-intake' ) ) : $backfill;
				break;
	...
	private static function repair_label( string $repair ): string {
		$labels = array(
			...
			'verify_unified_platform_migration' => __( 'Verify v2.0 integrated platform migration', 'sustainable-catalyst-engagement-intake' ),
			'repair_unified_platform' => __( 'Rebuild canonical engagement dossiers', 'sustainable-catalyst-engagement-intake' ),
