...
final class SC_EI_Hardening_Repository {
	...
	public static function watchdog( bool $manual = false ): array {
		...
		try {
			...
			$checks['support_columns']=!in_array(false,SC_EI_Database::support_columns_exist(),true);
			...
			foreach($checks as $key=>$passed) if(!$passed) self::record_event(in_array($key,array('database_tables','hardening_columns','workflow_core_columns','platform_columns','inquiry_columns','lifecycle_columns','support_columns'),true)?'database':(str_contains($key,'storage')?'storage':(str_contains($key,'transport')||str_contains($key,'crypto')?'security':'cron')), 'watchdog_'.$key, in_array($key,array('database_tables','hardening_columns','workflow_core_columns','platform_columns','inquiry_columns','lifecycle_columns','support_columns','storage_ready'),true)?'critical':'warning', 'Reliability watchdog detected a failed production readiness check.', array('check'=>$key));
