...
final class SC_EI_Diagnostics {
	...
	public static function run(): array {
		...
		return array(
			...
			'upload_note'        => __( 'Secure physical upload handling is scheduled for v0.3.0. v0.2.1 collects descriptions and non-confidential public links only.', 'sustainable-catalyst-engagement-intake' ),
			'teams'              => array(
	...
	public static function overall_status( array $results ): string {
		...
		$columns_ok = ! in_array( false, $results['teams_columns'], true );
		$caps_ok    = ! in_array( false, $results['capabilities'], true );

		return ( $tables_ok && $columns_ok && $caps_ok && $results['public_forms'] ) ? 'healthy' : 'attention';
	}
}
