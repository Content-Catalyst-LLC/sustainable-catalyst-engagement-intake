			...
			<div class="sc-ei-workflow-portal-list">
				<?php foreach ( $proposals as $proposal ) : $scope = json_decode( (string) $proposal['scope_json'], true ) ?: array(); $deliverables = json_decode( (string) $proposal['deliverables_json'], true ) ?: array(); $exclusions = json_decode( (string) $proposal['exclusions_json'], true ) ?: array(); $assumptions = json_decode( (string) $proposal['assumptions_json'], true ) ?: array(); ?>
					<article class="sc-ei-portal-card sc-ei-portal-workflow-card">
						...
						<?php elseif ( 'changes_requested' === $proposal['status'] ) : ?>
							<div class="sc-ei-portal-notice"><strong><?php esc_html_e( 'Changes requested.', 'sustainable-catalyst-engagement-intake' ); ?></strong> <?php esc_html_e( 'Sustainable Catalyst will prepare a new immutable proposal version for review.', 'sustainable-catalyst-engagement-intake' ); ?></div>
						<?php endif; ?>
						<?php foreach ( $statements_of_work as $sow ) : if ( absint( $sow['proposal_id'] ) !== absint( $proposal['id'] ) ) continue; $sow_scope = json_decode( (string) $sow['scope_json'], true ) ?: array(); $sow_deliverables = json_decode( (string) $sow['deliverables_json'], true ) ?: array(); ?>
							<section class="sc-ei-portal-notice"><h5><?php echo esc_html( $sow['sow_number'] . ' · v' . absint( $sow['version_number'] ) . ' — ' . $sow['title'] ); ?></h5><p><?php echo nl2br( esc_html( $sow['purpose_background'] ) ); ?></p><div class="sc-ei-portal-proposal-columns"><div><strong><?php esc_html_e( 'Scope', 'sustainable-catalyst-engagement-intake' ); ?></strong><ul><?php foreach ( $sow_scope as $item ) : ?><li><?php echo esc_html( $item ); ?></li><?php endforeach; ?></ul></div><div><strong><?php esc_html_e( 'Deliverables', 'sustainable-catalyst-engagement-intake' ); ?></strong><ul><?php foreach ( $sow_deliverables as $item ) : ?><li><?php echo esc_html( $item ); ?></li><?php endforeach; ?></ul></div></div><p><strong><?php esc_html_e( 'Acceptance criteria', 'sustainable-catalyst-engagement-intake' ); ?></strong><br><?php echo nl2br( esc_html( $sow['acceptance_criteria'] ) ); ?></p>
							<?php if ( 'approved' === $sow['status'] && ! $privacy_restricted && ! empty( $permissions['respond_proposals'] ) ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="sc-ei-portal-form"><input type="hidden" name="action" value="sc_ei_portal_approve_sow"><input type="hidden" name="portal_csrf" value="<?php echo esc_attr( $csrf_token ); ?>"><input type="hidden" name="sow_id" value="<?php echo esc_attr( $sow['id'] ); ?>"><label><span><?php esc_html_e( 'Approval note', 'sustainable-catalyst-engagement-intake' ); ?></span><textarea name="sow_response_note" rows="3"></textarea></label><label class="sc-ei-check"><input type="checkbox" name="proposal_authority_attested" value="1" required><span><?php esc_html_e( 'I am authorized to approve this Statement of Work.', 'sustainable-catalyst-engagement-intake' ); ?></span></label><label class="sc-ei-check"><input type="checkbox" name="proposal_boundary_acknowledged" value="1" required><span><?php esc_html_e( 'I understand this portal record supports engagement administration and does not replace separately required legal review or contracting.', 'sustainable-catalyst-engagement-intake' ); ?></span></label><label><span><?php echo esc_html( 'Type APPROVE ' . strtoupper( $sow['sow_number'] ) ); ?></span><input name="sow_confirmation" required autocomplete="off"></label><button class="sc-ei-button sc-ei-button--primary"><?php esc_html_e( 'Approve Statement of Work', 'sustainable-catalyst-engagement-intake' ); ?></button></form><?php else : ?><p><strong><?php esc_html_e( 'Statement of Work status:', 'sustainable-catalyst-engagement-intake' ); ?></strong> <?php echo esc_html( SC_EI_Proposal_Governance_Schema::sow_statuses()[ $sow['status'] ] ?? $sow['status'] ); ?></p><?php endif; ?>
							</section>
						<?php endforeach; ?>
					</article>
				<?php endforeach; ?>
